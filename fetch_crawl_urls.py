...
class UrlCrawler:
    def __init__(self, url, file_name, images_name, error_file):
        self.url = url
        ...
        try:
            async with session.get(url) as response:
                if response.status != 200:
                    with jsonlines.open(self.error_file, mode='a') as err_writer:
                        err_writer.write({"url": url, "error": f"Status {response.status}"})
                    return new_links
                ...
                if len(content) > 50 * 1024 * 1024:
                    with jsonlines.open(self.error_file, mode='a') as err_writer:
                        err_writer.write({"url": url, "error": "Page too large"})
                    return new_links
            ...
            with jsonlines.open(self.error_file, mode='a') as err_writer:
                err_writer.write({"url": url, "error": str(e)})
    ...
    def sync_crawl_single_url(self, url, base_netloc, visited_actor):
        ...
        try:
            ...
            if response.status_code != 200:
                with jsonlines.open(self.error_file, mode='a') as err_writer:
                    err_writer.write({"url": url, "error": f"Status {response.status_code}"})
                return new_links
    ...
    def scrape_all_links(self, max_pages=None):
        ...
        ray.get(visited.add.remote(start_url))
